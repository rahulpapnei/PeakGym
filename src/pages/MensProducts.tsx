...
// Define the interface inside the component file
interface Product {
  id: number;
  name: string;
...
// Instead of exporting the data array directly, create a function that returns it
...
// Use useMemo for priceRange
const priceRange = useMemo(() => [0, 6000] as [number, number], []);

// Get products using useMemo to avoid unnecessary recalculations
const products = useMemo(() => getMensProducts(), []);
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 navbar-spacing">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
      {filteredProducts.map((product) => (
        <div
          key={product.id}
          className="bg-card rounded-lg shadow-lg overflow-hidden cursor-pointer transform transition-transform hover:scale-105 border border-border"
        ...
        >
          <div className="relative">
            ...
            />
            <div className="absolute top-4 right-3 bg-primary text-background px-3 py-1 rounded-full flex items-center">
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                {product.discount > 0 ? (
                  ...
                  <span className="text-2xl font-bold text-primary">₹{product.originalPrice}</span>
                )}
              </div>
              {product.discount > 0 && (
                <span className="text-sm bg-accent text-background px-2 py-1 rounded">
